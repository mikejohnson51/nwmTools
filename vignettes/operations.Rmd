...
system.time({ create_nwm_nc(fileList = in_files, dstfile = nco_pivot) })

# Use NCO but do not pivot
system.time({ create_nwm_nc(fileList = in_files, dstfile = nco_nopivot, pivot = FALSE) })

# Use RNetCDF
system.time({ create_nwm_nc(fileList = in_files, dstfile  = r_nopivot, nco = FALSE) })
```
...
AOI =  get_nhdplus(aoi_get("Colorado Springs")) %>% 
  select(comid)
...
system.time({
  ...
  forecast = extract_nwm(nwm, comids = AOI$comid)
})
