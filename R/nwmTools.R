...
#' @importFrom RNetCDF open.nc close.nc create.nc var.get.nc var.inq.nc var.def.nc var.put.nc dim.inq.nc  dim.def.nc att.get.nc att.put.nc    
#' @importFrom terra values rast nlyr writeCDF ext crs align ext project vect ymax ymin xmin xmax flip crop sds `time<-`
#' @importFrom glue glue
#' @importFrom dplyr mutate filter left_join group_by_at add_tally summarize_at vars ungroup `%>%` select 
